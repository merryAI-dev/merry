...
function buildMarkdown(result: ParseResult): string {
  if (!result.ok) return `오류가 발생했습니다: ${formatParseError(result.error)}`;
...
function formatParseError(error?: string): string {
  switch (error) {
    case "UNAUTHORIZED":
    case "UNAUTHORIZED:NO_SESSION":
    case "UNAUTHORIZED:NO_TOKEN":
    case "UNAUTHORIZED:INVALID_TOKEN":
    case "UNAUTHORIZED:DOMAIN_MISMATCH":
