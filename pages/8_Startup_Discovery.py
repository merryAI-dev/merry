...
# Discovery 전용 포트폴리오 결과 캐시
if "discovery_portfolio_result" not in st.session_state:
    st.session_state.discovery_portfolio_result = None


def _render_portfolio_result_section():
    result = st.session_state.get("discovery_portfolio_result")
    if not result:
        return
    st.markdown("### 투자기업 포트폴리오 검색 결과")
    ...
    if not records:
        ...
        return
    st.dataframe(pd.DataFrame(records).head(15))
...
if st.session_state.discovery_policy_analysis or st.session_state.discovery_recommendations:
    ...
    with tab5:
        ...
        if user_input:
            ...
            with st.chat_message("assistant", avatar=avatar_image):
                ...
                try:
                    ...
                    if agent_for_portfolio:
                        st.session_state.discovery_portfolio_result = getattr(agent_for_portfolio, "last_portfolio_result", None)
        ...
        _render_portfolio_result_section()
