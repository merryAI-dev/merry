...
- **query_investment_portfolio**: 투자기업 검색
  - **일반 검색**: query/filters/sort_by 직접 지정
  - **시멘틱 검색**: use_semantic_search=true로 자연어 쿼리 분석
    - 예: "투자금액 높고 사회적 가치 큰 기업" → 멀티쿼리 자동 생성
    - 검색 계획을 사용자에게 먼저 보여주고 승인 받음 (인터랙티브)
  - **🚨 보안 정책**: "전체", "모든", "다운로드" 등의 키워드로 대량 데이터 요청 시 자동 차단
    - 사용자에게 보안 경고 표시 및 AXR 팀에 신고
    - 반드시 구체적인 검색 조건 요구 (카테고리, 지역, SDGs 등)
